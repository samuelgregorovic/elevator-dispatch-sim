...
**A21. Service level, efficiency, and seconds.** Besides the distributions, every summary reports the share of passengers who waited more than 30 ticks — the industry's "waits over a minute" service level, at the two seconds per floor a real car manages — and two efficiency figures: intermediate stops sat through per delivered passenger (stops the car made between that passenger's boarding and alighting) and floors of car travel per passenger delivered. Ticks are the unit everywhere; where a document gives seconds it is this two-second conversion, stated as such, not a measurement.

**A22. Time-of-day parking.** `park_schedule` is a list of `(from_tick, floor)` pairs; from each tick on, idle cars return to that floor, overriding `park_floor`. Every scheduled floor must be served by every car. Reason: a single park floor is right for one traffic direction and wrong for the other; the office-day study shows a fixed lobby park is neutral over a day while a schedule pays.
