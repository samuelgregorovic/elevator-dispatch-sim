...
import { Simulation, makeScheduler, parseCsv, carUsage, service, efficiency } from './engine.js';
...
const config = {
  ...
  dwellTicks: cfg.dwell_ticks ?? 1, startFloor: cfg.start_floor ?? 1, parkFloor: cfg.park_floor ?? null, parkSchedule: cfg.park_schedule ?? [], servedFloors,
};
...
process.stdout.write(JSON.stringify({
  ...
  service_over: service(sim).over,
  intermediate_stops: efficiency(sim, carUsage(sim)).intermediate,
}));
